...
| **Review** | Extend the smoke test to cover scenarios 02 and 03. | Add three checks, as scoped. | **EDITED** | Three was the estimate, not the requirement, and stopping there would have left the gap it was meant to close. Eleven added: expiry needs the active resolve, the `410`, the **absent `Location`**, the `EXPIRED` status, and the frozen counter — drop any one and a broken implementation still passes. Reliability adds the two published signals, the actuator surface, and a destination-leak canary. The script had described itself as the "full reviewer path" while testing only Scenario 01, which is the kind of claim that makes a green result actively misleading. | 36/36 against `docker compose`. | Engineer approved. |
| **Review** | Compute a near-future expiry timestamp in the smoke script. | `date -u -d "+5 seconds"`. | **REJECTED** | GNU syntax on a script that runs on macOS, where BSD `date` wants `-v+5S` — and BSD `date -d` does not fail loudly, it means something else entirely, so the script would have produced a wrong timestamp rather than an error. Detected by probing `date --version` rather than branching on `uname`, because the question is which implementation is on PATH and coreutils on macOS makes those different questions. **The same assumption already broke this repository once**, when `mapfile` was used on a machine shipping bash 3.2. | Ran on BSD `date`; both branches exercised by inspection. | Engineer approved. |
| **Review** | Confirm the new checks pass. | 36/36 green, reported as done. | **REJECTED** | A green check that cannot go red proves nothing, and two of these were built to look exactly like that. Verified both by negative control: a marker planted into the metrics payload **is** caught by the leak grep, and the `Location` parser returns a real value for an active link — so the empty result asserted on the `410` is a measurement rather than a parsing artifact. Reporting green without that would have shipped two decorative assertions into the one script a reviewer actually runs. | Negative-control output recorded in the session. | Engineer approved. |
